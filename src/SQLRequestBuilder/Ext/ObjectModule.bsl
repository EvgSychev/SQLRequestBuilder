...
Функция ПолучитьНужныйУзел(Знач вхТокены="")
	...
	Пока НЕ ТипУзелаДерево(РезультатФункции) Цикл
	...
	Если ЗначениеЗаполнено(вхТокены) И ТипУзелаДерево(РезультатФункции) Тогда
...
Функция БлокЗакрыт(Узел)
	...
	ИначеЕсли Узел.Токен = "IF" Тогда
		РезультатФункции = Узел.Строки.Количество() > 0;
	ИначеЕсли Узел.Токен = "WHILE" Тогда
		РезультатФункции = Узел.Строки.Количество() > 0;
...
Функция Index(Знач ИмяИндекса, Знач Кластерный=Ложь, Знач Уникальный=Ложь) Экспорт
	...
	Если Уникальный Тогда
	    МассивСтрок.Добавить("unique");
	КонецЕсли; 
	
	Если Кластерный Тогда
		МассивСтрок.Добавить("clustered");
	КонецЕсли;
	...
	ДобавитьУзел(ТекущийУзел, "INDEX", СтрСоединить(МассивСтрок, " "));		
...
Функция FetchNext() Экспорт
	ДобавитьУзел(ТекущийУзел, "FETCHNEXT", "fetch next");
	Возврат ЭтотОбъект;
КонецФункции // Next()
