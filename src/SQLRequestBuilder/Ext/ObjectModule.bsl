...
Процедура Инициализация() Экспорт
	...
	ДеревоAST.Колонки.Добавить("ЛевыйСимволГруппы", Новый ОписаниеТипов("Строка"));
	ДеревоAST.Колонки.Добавить("ПравыйСимволГруппы", Новый ОписаниеТипов("Строка"));
...
Функция CreateTable(Знач ИмяТаблицы) Экспорт

	ТекущийУзел = ПолучитьНужныйУзел();

	ДобавитьУзел(ТекущийУзел, "CREATETABLE", СтрШаблон("create table %1", ИмяТаблицы));

	ВиртуальныйКонтейнер(ТекущийУзел, "COLUMNSDEFINITION", ", ");
	ТекущийУзел.ЛевыйСимволГруппы = "(";
	ТекущийУзел.ПравыйСимволГруппы = ")";
	
	Возврат ЭтотОбъект;

КонецФункции // CreateTable()

Функция ColumnDefinition(Знач Имя, Знач Тип) Экспорт
	
	ТекущийУзел = ПолучитьНужныйУзел("COLUMNSDEFINITION");

	ДобавитьУзел(ТекущийУзел, "COLUMN", СтрШаблон("%1 %2", Имя, Тип));

	Возврат ЭтотОбъект;

КонецФункции

Функция ColumnOptions(Знач Опции) Экспорт
	
	ТекущийУзел = ПолучитьНужныйУзел("COLUMN");

	ВиртуальныйКонтейнер(ТекущийУзел, "COLUMNOPTIONS", " ");

	ОбработатьПараметрСписок(Опции, "ColumnOption");

	Возврат ЭтотОбъект;

КонецФункции

Процедура ColumnOption(Знач Опция)
	ДобавитьСтроку(ТекущийУзел, "COLUMNOPTION", Опция);
КонецПроцедуры

Функция CreateIndex(Знач ИмяИндекса, Знач ИмяТаблицы) Экспорт

	ТекущийУзел = ПолучитьНужныйУзел();
	
	МассивСтрок = Новый Массив;	
	МассивСтрок.Добавить("create index");
	МассивСтрок.Добавить(ИмяИндекса);
	МассивСтрок.Добавить(Символы.ПС);
	МассивСтрок.Добавить("on");
	МассивСтрок.Добавить(ИмяТаблицы);
	
	ДобавитьСтроку(ТекущийУзел, "CREATEINDEX", СтрСоединить(МассивСтрок, " "));		
...
Функция IndexColumns(Знач Колонки) Экспорт
	
	ТекущийУзел = ПолучитьНужныйУзел("CREATEINDEX");
	
	ДобавитьСтроку(ТекущийУзел, "INDEXCOLUMNS", СтрШаблон("(%1)", Колонки));
